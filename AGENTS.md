...
- タスク分解が完了したら、実装着手前にレビューを挟み、承認を得てから実装に進む。
- 要件変更・設計変更が発生した場合は、コード変更より先に文書を更新する。
- 実装後は、関連する文書とテストを同期する。
- 実装を伴うタスクは、コード反映と確認（テストまたは手順確認）が完了するまで `active` に残す。
...
5. 分解したタスク計画をレビューし、承認を得る
6. タスク単位で実装する
7. テスト・確認を行う
8. 文書を同期する
9. 完了したタスクを `docs/dev/tasks/done/` に移動する
