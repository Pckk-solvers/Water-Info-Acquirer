...
- 対象となる値の種類は水位・流量とし、内部列名は対象種別に依存しない。列名ゆらぎは読み込み時に吸収する。

## 対象種別と表示ラベル

- 後処理の対象種別は、水位 (`water_level`) と流量 (`discharge`) を扱う。
- `--metric` で対象種別を指定でき、省略時は従来互換のため `water_level` とする。
- 計算用の内部列名は種別に依存しないが、Excelの見出し・位況/流況名・ピーク名・平均/最大/最小の表示ラベルは対象種別から動的に生成する。
  - 水位: `位況豊水位`、`位況平水位`、`位況低水位`、`位況渇水位`
  - 流量: `流況豊水流量`、`流況平水流量`、`流況低水流量`、`流況渇水流量`
- 雨量の取得は既存の取得GUIで扱い、位況・流況後処理GUIの対象には含めない。

## 後処理GUI

- 取得済みの時間データ (`_H` Excel) を必須入力とする。
- 日データ (`_D` Excel) は任意入力とする。
- 対象種別（水位/流量）、Excel出力先、任意のParquet出力先をGUIで指定できる。
- 時間データのファイル名が `_WH` または `_QH` の場合、対象種別の初期値を推定する。ただし、最終的な対象種別はGUIの選択値を正本とする。
- 対象種別を変更したとき、画面の説明・ラベル例・処理結果の表示名を即時に更新する。
- 取得処理はこのGUIでは行わず、既存の水文データ取得GUIで取得したファイルを入力する。
...
## 位況算出
- 基準順位: 豊水位/豊水流量=95、平水位/平水流量=185、低水位/低水流量=275、渇水位/渇水流量=355 をベースに 365 日基準で設定。
- ランク補正 `_calc_rank`:
...
- 列: `peak_max_value`, `peak_max_time`（時刻は水文水質DB基準）。値は後段で小数第2位に丸めて出力。
...
- 列名は Excel 出力時に日本語へリネームする（例: 水位なら日平均水位・位況渇水位、流量なら日平均流量・流況渇水流量）。数値は書き出し前に小数第2位へ `ROUND_HALF_UP`。
...
- 日データなし:
  ...
  ```
- 流量（流況）として処理する場合は、同じ入力に `--metric discharge` を追加する。これは取得処理ではなく、取得済みExcelの後処理だけを実行する。
