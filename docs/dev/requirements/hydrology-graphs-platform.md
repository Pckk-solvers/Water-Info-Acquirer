...
- 編集手段:
  ...
  - フォーム編集項目に `表示データ範囲トリム` を含める（`x_axis.data_trim_start_hours` / `x_axis.data_trim_end_hours`）
    - 単位は時間、現行は `0.5` 単位で入力・検証できる
  - 値入力行は原則チェックボックスでON/OFFを持ち、OFF時は値を保持したまま編集のみ無効化する
...
- イベント系:
  ...
  - 現行実装は対象窓（3日/5日）で欠損なし前提（`value` 欠損、`quality=missing` はNG）だが、欠測方針は 10 章の移行要件を正本として段階的に切り替える
- 表示モード:
...
- X軸範囲マージン率:
  ...
  - 値は `0` 以上の数値のみ許可する
- 表示データ範囲トリム:
  - `x_axis.data_trim_enabled` が `true` のときだけ適用する
  - style JSON の `graph_styles.<key>.x_axis.data_trim_start_hours` / `data_trim_end_hours` を正本とする
  - 未指定時の既定値はどちらも `0.0` とする
  - 値は `0` 以上の数値のみ許可する
  - 描画前データから先頭/末尾を除外し、累積雨量は除外後データで再計算する
- 年最大系:
...
- 欠測判定に関する最終的な仕様決定は本章を優先し、5章の現行記述は移行前提として扱う。
- 欠測判定は以下の3分類を使い、precheck / preview / batch / render / UI で同じ語彙を使う。
  - `ok`: 欠測なし、または欠測があっても継続可
...
- `M1: 行欠落`
  - 期待時刻（1hour）に行が存在しない。
- `M2: quality欠測`
  - 行は存在するが `quality=missing`。
- `M3: value欠測`
  - 行は存在するが `value` が `null/NaN`。

#### 10.5.2 判定ルール（イベント系）
- `M1/M2/M3` が窓内に存在しない: `ok`
- `M1/M2/M3` が窓内に存在するが、描画可能な最小条件を満たす: `warn`
  - 描画可能な最小条件:
    - 時刻列が解釈可能であること
    - 対象窓に1件以上の有効データ（`value` 数値かつ `quality!=missing`）があること
- 上記最小条件を満たさない: `ng`

#### 10.5.3 判定ルール（年最大系）
- 入力に欠測があっても、年最大算出可能で年数要件を満たす: `warn`（欠測あり）または `ok`（欠測なし）
- 年最大算出不能、または年数要件不足: `ng`

#### 10.5.4 可視化と表示義務
- `warn` の場合:
  - render: 欠測区間を可視化（欠測帯、線切断）
  - UI: 結果一覧・ログで「欠測あり継続」を識別表示する
- `ng` の場合:
  - 欠測理由を種別（`M1/M2/M3`）に紐づけて表示する

## 11. UIリファクタリング要件（非機能）

### 11.1 背景
- `src/hydrology_graphs/ui/app.py` に責務が集中しており、仕様変更時の影響範囲が広くなっている。

### 11.2 目的
- UIの責務を分離し、保守性・可読性・変更安全性を改善する。

### 11.3 スコープ
- `src/hydrology_graphs/ui/` 内のモジュール分割と委譲構造の整理。
- 既存のユーザー向け挙動は変更しない。

### 11.4 制約
- 段階的に分割する（1タスクで完結する粒度）。
- 先に文書（tasks/architecture）へ分割境界を記述してから実装する。
- 各段階で部分テストを通してから次へ進む。

### 11.5 完了条件
- `app.py` は coordinator（状態保持 + 委譲）中心の責務に縮小されている。
- 分割後も `style/preview/execute` の挙動差分がない。
- `tests/hydrology_graphs/test_ui_support.py` と `tests/hydrology_graphs/test_preview_actions.py` が通過する。

## 12. ハイドログラフ比較モード

### 12.1 目的
- ハイドログラフ（流量・水位）において、2つの観測所データを同一グラフ上に重ねて比較描画できるようにする。
- 異なる地点間（上流・下流）や異なる性質のデータ間の相関を視覚的に確認可能にする。

### 12.2 スコープ
- 対象グラフ: `hydrograph_discharge`, `hydrograph_water_level`
- ハイエトグラフおよび年最大系グラフは比較対象外とする。

### 12.3 スタイル設定
- `graph_styles` に `series2` オブジェクトを追加し、比較系列（2本目）の見た目を制御する。
- 設定項目:
  - `enabled`: 2本目の表示ON/OFF
  - `color`: 線の色
  - `width`: 線の太さ
  - `style`: 線種（実線、破線など）
  - `use_secondary_y`: 右軸（Y2軸）を使用するかどうか
- 比較モードが有効な場合、自動的に凡例（Legend）を表示し、それぞれの系列がどの観測所のものか識別可能にする。

### 12.4 UI
- スタイル調整タブの「プレビュー出力対象」に「比較先」選択ドロップダウンを追加する。
- スタイル調整パレットに「比較系列(2本目)」の設定行を追加する。
- ハイドログラフ以外のグラフ種別を選択した際は、比較関連のUI設定行を自動的に非表示にする。

### 12.5 完了条件
- スタイル調整タブで「比較系列(2本目)」の設定がプレビューに反映される。
- 異なる観測所を重ねて表示し、凡例で識別できる。
- Y2軸（右軸）への切り替えが正常に機能する。
- 既存の単一系列表示の挙動に影響を与えない。
