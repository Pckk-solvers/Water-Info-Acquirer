...
## ハイライト
- ランチャーを新設し、water_info / JMA を単一Tkルート＋子ウィンドウ（Toplevel）で切り替えられるように統合。PyInstallerビルド後も `main.py` からワンクリックで起動できます。
- アプリ名称・バージョンを `src/app_names.py` と `pyproject.toml` に一元管理。ランチャー上にバージョンを表示し、メニューラベルも共通の定義を参照。
- JMA GUIの初期化を非同期化（都道府県一覧の取得をバックグラウンド化）し、オーバーレイ表示で進行状況を可視化。起動待ち時間を短縮しました。
...
- JMA `browse_tab.py`:  
  - 都道府県一覧取得を非同期化し、リスト上にオーバーレイで「読み込み中」を表示。  
